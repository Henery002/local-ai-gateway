...
`Local AI Gateway` v1 是一个运行在 macOS 本机的 AI 网关系统，优先服务 OpenClaw 的本地接入场景。

产品定位：

- 本地单用户使用
- 本机常驻运行
- 对 OpenClaw 暴露统一 AI 接口
...
目标用户是同一台 Mac 上长期运行 OpenClaw、Codex 等工具的单个本地用户。
...
当 OpenClaw 将 `baseUrl` 配置为 `http://127.0.0.1:8787/v1` 后，应能稳定完成以下链路：
