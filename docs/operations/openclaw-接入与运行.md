...
- 仅修改源码后，当前“已在运行中的网关进程”不会自动热更新。
- 要让优化生效，至少需要重启网关进程（开发态可直接重启 `npm run dev:gateway` 或在桌面端执行“重启服务”）。
- 只有在你需要更新安装版 `.dmg/.zip` 给他人部署时，才需要重新打包；本机联调不要求每次都打包安装。

简化结论：

- `重启服务`：让本机当前网关获得新能力（必须）
- `重新打包安装`：产出新安装包给分发场景（可选）
...
- 网关会对上游返回的 `401/403` 或错误标签（如 `[status:403]`）按鉴权失效处理，失败账号会进入较长冷却窗口，优先切换到下一可用账号。
- 网关会吸收上游 `retry-after` 提示（如 `[retry-after:180]`），用于设置成员最短冷却时间，减少短时间反复命中同一受限账号。
