...
## [2026-06-02 10:40 CST]

### 修复

- 修正 `单账号耗尽后切换` 号池策略：优先粘住最近一次成功请求的账号，只有额度耗尽或鉴权失效才持久冷却并切换；临时限流、网络或上游重试类失败只影响当前请求内重试，不再把账号长期打入冷却。

## [2026-06-01 14:35 CST]

### 文档

- 追加 Cockpit 侧 Codex 账号池 2026-06-01 停用 API 服务前基线：当前 `136` 个 OAuth 账号中 `47` 个授权异常，全部为 `token_invalidated`，并记录从本次观测后暂停 Cockpit 侧 API 服务、改用本网关 `单账号耗尽后切换` 策略作为后续风控对照实验分界点。

## [2026-05-31 10:00 CST]

### 文档

- 追加 Cockpit 侧 Codex 账号池 2026-05-31 复查记录：当前 `136` 个 OAuth 账号中 `44` 个授权异常，全部为 `token_invalidated`，并补充邮箱后缀异常率、与 2026-05-30 的重叠/新增/脱出、`expiry_soon_first` 切号策略风险和迁移到本网关后的保守限流建议。

## [2026-05-30 17:15 CST]

### 文档

- 追加 Cockpit 侧 Codex 账号池 2026-05-30 复查记录：当前 `137` 个 OAuth 账号中 `38` 个授权异常，并按 `192168系列`、`dsai系列`、`其他系列`、`gmail系列` 统计异常率、与 2026-05-29 的重叠/新增/脱出及规律分析。
- 补充账号池用途与使用强度解释变量：`192168系列` + `dsai系列` 属于 Cockpit API 服务高频号池，`其他系列` 属于公网网关低频号池，后续异常率分析需同时考虑暴露度差异。

## [2026-05-29 13:30 CST]

### 文档

- 追加 Cockpit 侧 Codex 账号池 2026-05-29 大面积授权过期复查记录：当前 `137` 个 OAuth 账号中 `35` 个为 `token_invalidated`，并补充与 2026-05-28 异常账号的重叠、批次分布和规律分析。

## [2026-05-28 12:00 CST]

### 修复

- 修复“测试 API 服务”误报失败：自检请求优先使用已验证可用的 `codex-5.5` / `gpt-5.5`，避免旧默认 `gpt-5.4` 在 ChatGPT 账号下不支持导致自检失败；系统默认 Codex 上游模型同步调整为 `gpt-5.5`。
- 管理面服务自检失败不再写入成员访问告警，避免 `desktop-service-test` 的诊断请求污染三方用户可用性判断；真实运行失败仍保留运行日志。

### 优化

- 公网用户自助查询页升级为登录态 + 用量看板态：登录页复刻 `animated-characters-login-page` 的角色跟随、输入凝视、显示 Key 偷看和随机眨眼动效；登录后进入单成员用量页，页面结构、类名、统计图表类型和交互布局进一步对齐桌面端“用量与告警”，并提供退出登录入口清空浏览器内存中的 API Key。

### 文档

- 更新 Cockpit 侧 Codex 账号池长期观测记录：补充 2026-05-28 全量状态复查，以 `137` 个 Codex OAuth 账号为分析口径，排除 `1` 个 API Key / custom 服务账号，并记录 `14` 个 `token_invalidated` 异常及相较 2026-05-24 基线的变化。
- 账号池观测记录补齐 14 个异常账号的邮箱标识，并记录本轮二次授权进入原注册手机号验证、JP 一次性虚拟手机号不可控导致常规恢复阻断的重大维护变故及后续账号池治理策略。
- 追加 2026-05-28 晚间复查：日志核验下午出现 4 次 `token_revoked`，二次 OAuth 后当前授权异常从 14 个降至 3 个，11 个账号通过邮箱验证码恢复，3 个账号仍卡原 JP 手机号验证码。

## [2026-05-27 00:00 CST]

### 新增

- 新增公网用户自助用量 / 额度查询 MVP：`/v1/account` 提供轻量查询页，`/v1/user/profile` 返回当前成员 Key 的脱敏画像和额度摘要，`/v1/user/usage/summary` 按当前 `consumerId + accessKeyId` 隔离返回本人用量趋势与模型排行。
- 新增《公网用户自助用量额度查询页技术方案》和实施清单，明确继续只暴露 `/v1/*` 公网面，禁止开放 `/admin/*`，且不返回完整 API Key、`keyHash`、OAuth token、上游账号邮箱或 prompt 正文。
- 公网用户自助查询页完成整体现代化重构：撤换临时生成式落地页风格，改为复用桌面控制台同源设计 token、白底运营卡片、左侧成员 Key 面板、右侧用量观测大盘和“用量与告警”式趋势 / 构成 / 排行布局。
- 参考 Cockpit API 服务自检交互，新增桌面端“测试服务”入口与 `/admin/service/test` 管理接口，可通过本地网关发起一次极小真实推理请求，返回脱敏的结果、模型、Provider、HTTP 状态和延迟。
- 号池调度策略新增“优先近到期”（`expiry-asc`），优先消耗额度重置时间或授权到期时间更近的账号；未知到期时间账号自动排后，避免浪费快到期额度。
- 号池调度策略新增“单账号耗尽后切换”（`single-drain`），并作为新建号池默认策略：首次从可用候选中选剩余额度最高账号，后续持续使用当前已选账号，只有额度低于阈值、进入冷却、鉴权失败或本次请求已尝试过时才切到下一个候选，降低短时间同出口 IP 高频轮转多个账号的暴露面。

### 优化

- 运维日志页顶部按钮组再次收敛为“一键修复公网链路 / 安装并启动常驻 / 刷新”，网关启动、停止、重启和 Tunnel 重启下放到对应状态卡片内；同时清理顶部状态卡下方空白占位。
- 账号资产页排序维度扩展为剩余额度、重置时间、最近活跃、请求数和名称；账号操作列从图标按钮改为文字按钮，并移除“置顶”操作入口，避免操作语义不清。
- 用量页“成员健康度”卡片补充最近活跃时间，和成员健康矩阵的最近活跃列保持一致。
- 页面切换会回到当前页顶部；弹窗打开时默认回到弹窗顶部，访问成员编辑弹窗的内部刷新仍保留原滚动位置，避免编辑中跳动。
- 弹窗遮罩关闭规则收敛：只读查看类弹窗支持点击遮罩关闭；账号导入、成员新增 / 编辑、号池新增 / 编辑和告警阈值设置等编辑类弹窗不再因误点遮罩关闭。
- 模型配置页 Provider 卡片改为单列独占一行，避免 OpenAI-Compatible 与 Ollama 表单在宽屏下并排造成扫描负担。
- 运维日志页移除“高级服务控制”独立卡片，把安装常驻、启动、停止网关集成到页面顶部按钮组；账号健康与路由解释新增剩余总额度、额度风险、号池覆盖等语义色 KPI。
- 系统诊断页再次重构系统配置布局，改为双列配置台、重点卡片、12 栅格表单和统一按钮对齐，保留原有鉴权、共享入口、客户端映射、桌面运行与数据迁移行为。
- 收敛桌面端自动刷新交互：后台额度同步不再在每次自动刷新后弹出成功 / 失败横幅，降低正常使用时的打断感。
- 静默运行态刷新在账号资产页不再重建账号表格，只更新顶部摘要和图表，避免使用中出现整页闪跳。
- 账号资产和号池调度的批量勾选改为局部同步复选框、选中数量和按钮状态，不再因单次勾选销毁重建 Grid.js 表格。
- 用量与告警页新增独立“刷新用量”按钮，用于即时刷新 Token 用量、告警和图表；顶部“刷新状态”继续保留为全局状态与实时额度同步入口。

## [2026-05-26 11:50 CST]

### 调整

- 顶部状态栏图标在 macOS 浅色 / 深色模式下统一使用白色实体图标，避免浅色桌面菜单栏中出现突兀黑色图标；保留公网请求活跃时的逐帧动效。
- 继续精简顶部状态栏下拉菜单：摘要区收束为网关健康、运行状态、公网入口、实时 / 最近请求、近 30 分钟用量、号池与额度等高价值观测项。
- 移除托盘菜单中的“复制公网 Provider 地址”入口，保留打开控制台、重启网关服务、退出控制台、停止公网链路并退出等必要动作，降低菜单按钮噪音。

## [2026-05-26 00:12 CST]

### 修复

- 调整 macOS 退出语义：关闭窗口、Dock 退出和 `Command+Q` 默认只隐藏到菜单栏，保留顶部状态栏图标且不停止公网链路；顶部菜单和托盘新增“退出控制台（公网链路继续运行）”与“停止公网链路并退出...”两个明确动作。
- 修复 macOS 顶部状态栏图标在深色/蓝色菜单栏下几乎不可见的问题：暗色菜单栏托盘资源改为白色实体图标，并取消对系统 template 着色的依赖。
- 恢复顶部状态栏为纯图标样式，撤回临时 `RG` 文本、固定 Tray `guid` 与右上角兜底浮窗；最终确认菜单栏不展示的根因在 macOS“系统设置 - 控制中心 - 菜单栏 - 允许在菜单栏显示”开关与菜单栏背景显示状态。
- 重构顶部状态栏下拉菜单：将旧的账号视角摘要调整为公网中转视角，优先展示公网入口、链路状态、近期用量、访问成员、调度号池和上游额度，并补充复制公网 Provider 地址入口。
- 安装版本机网关 LaunchAgent 改为优先使用本机 Node 运行常驻服务，避免 Electron `ELECTRON_RUN_AS_NODE` 模式下网关进程出现 100% CPU、监听端口但 HTTP 无响应的问题；Electron 仅负责桌面控制台与顶部状态栏入口。
- 常驻网关 LaunchAgent 迁移到 `com.relaygate.gateway`，旧 `com.local-ai-gateway.gateway` / devsource label 会作为遗留服务禁用清理，避开旧 label 在 launchd 中进入 `spawn scheduled` 卡滞后无法立即恢复的问题。
- 安装版启动时即使发现旧网关进程占用 8787 且健康检查无响应，也会接管并重建常驻服务，不再误把“本地网关进程存在”当作“无需修复”。
- 桌面端 `/healthz` 探测补充 1.5 秒超时，避免旧网关处于“端口可连接但 HTTP 不返回”时卡住桌面端启动与一键修复流程。
- 桌面端网关 `ensureRunning()` 并发锁前移，避免多个初始化请求同时触发常驻服务 `bootout/kickstart`，造成网关刚启动又被另一个启动流程终止。
- 常驻服务启动/恢复探针由重型 `/healthz` 改为轻量 `/v1/models`，无 Key 返回 401 也视为网关存活，避免观测快照较重时被桌面端误判并反复重启。
- 新增轻量内部存活探针 `/__relaygate/livez`，桌面端常驻服务健康检查改走该端点，避免用 `/v1/models` 探活时把无 Key 的 401 记录成请求失败并污染告警/审计日志。
- 顶部状态栏后台刷新间隔从 1.2 秒放宽到 15 秒，避免频繁拉取完整 Admin 健康/号池/会话观测导致网关 CPU 长时间偏高。
- “停止公网链路并退出...”会二次确认，并在无活跃推理请求时依次停止 Cloudflare Tunnel 与本机网关 LaunchAgent，避免误触导致 Trae / Codex 公网请求中断。
...
- 品牌与总览顶栏完成公网化收口：桌面端可见品牌从 `Local AI Gateway / 本地 AI 网关` 调整为 `RelayGate / 公网中转网关`，更贴近当前公网 Provider 中转、成员治理和号池运营定位。
- 侧栏品牌图标移除外框与内边距，把原 Figma 图标内容放大到品牌位完整尺寸；左下角重构为 `Control Plane` 品牌状态卡，并加入系统最近更新日期。
...
- 号池调度页新增池内“可用额度”观测：号池列表和成员选择弹窗均展示当前号池内可用账号的总额度百分比，便于判断 public-ready 号池的上游额度充盈度。
- 本轮继续复用现有本地持久化用量汇总、账号额度快照和号池运行态；只增强桌面端展示与测试断言，不迁移、不重置、不删除任何成员、Key、账号、号池或历史用量数据。
- 总览页 Token 图表区继续增密：在主 Token 消耗结构柱图下方新增“Token 构成占比”环图和“主要消耗来源”横向排行图，分别观察输入 / 输出 / 缓存 / 思考占比，以及成员优先、客户端兜底的主要消耗来源。
- 新增图表继续复用当前窗口本地持久化用量汇总；ECharts 可用时渲染图表，不可用时回退为同风格数据列表，不改变任何用量存储或统计口径。

## [2026-05-25 13:28 CST]

### 调整

- 总览页重构为公网网关运营驾驶舱：第一屏突出 Public Base URL、公网成员、可用 Key、public-ready 号池、当前窗口 Token / 请求 / 失败 / 未确认告警，弱化旧版本机 / LAN 骨架。
- 重新组织总览页工作区：保留 ECharts Token 消耗结构图，新增成员与共享摘要、告警与治理摘要、接入面状态和路由命中观测，减少旧版 11 张碎片化小卡和大段快速指引堆叠。
- 总览页数据继续复用本地持久化用量事件、访问成员 / Key 配置、号池配置、账号健康快照和访问告警事件；本轮只改桌面端展示层，不迁移、不重置任何业务数据。

## [2026-05-25 13:18 CST]

### 调整

- 通知中心 Grid 表格移除行内“标为已读 / 确认告警”操作列，把宽度让给成员与消息内容；未读处理继续通过顶部批量已读入口完成，告警治理回到用量告警事件等专门入口。
- 修复通知类型徽标在窄列中不换行导致的溢出：类型列加宽并允许徽标多行居中显示。

## [2026-05-25 13:11 CST]

### 调整

- 左侧导航完成产品化命名与顺序收口：调整为“总览 / 成员与密钥 / 用量与告警 / 通知中心 / 账号资产 / 号池调度 / 模型配置 / 运维日志 / 系统诊断”，把成员治理、用量观测和通知前置，更贴近公网共享运营路径。
- 同步更新页面标题、跨模块提示文案和桌面构建回归测试；保留原 `data-nav-target` 与视图标识，不改变路由逻辑、IPC、成员、Key、账号资产、号池或用量数据。

## [2026-05-25 13:02 CST]

### 修复

- 修复“新增 / 编辑成员”弹窗 Key 管理区溢出：每条 API Key 独占完整行，Key 元信息与到期时间 / 暂停 / 轮换 / 删除操作分区展示，避免按钮挤出弹窗。
- 修正成员限制表单垂直对齐：请求数、并发、输入 / 输出 Token 和策略到期时间按 6 栅格重新排布，第二行字段宽度与顶部字段对齐。
- 访问成员 Grid 表格新增独立“状态”列，“启用 / 暂停”不再混入操作栏；操作栏只保留编辑、启停和删除动作。

### 调整

- 除账号资产表格外，Grid 表格操作栏按钮统一改为无边框文字按钮，默认横排，减少表格内按钮的视觉噪音。

## [2026-05-25 12:41 CST]

### 修复

- 修正 Grid.js 分页控件位置：每页条数选择移动到表格 footer，和 Grid.js 分页器一起在表格底部右侧展示，不再占用表格顶部空间。
- 加强 Grid.js 表头样式覆盖：同步覆盖 Grid.js 原生 `gridjs-th` 与 `gridjs-th-content`，确保表头加粗、居中生效。

### 调整

- 再次优化“新增 / 编辑成员”弹窗布局：成员信息改为稳定三列网格，备注横跨两列；额度与限制分组对齐，限制项独立分隔；API Key 管理改为左侧新增 Key、右侧 Key 列表的双列布局。

## [2026-05-25 12:29 CST]

### 调整

- Grid.js 表格规范继续增强：所有 Grid 表格统一默认每页 20 条，并在表格顶部提供 20 / 50 / 100 / 200 每页切换；搜索框统一靠右展示。
- 重新校准各 Grid 表格列宽：选择、状态、级别、请求数、延迟等短字段列收窄，成员、账号、消息内容、运行态等长字段列保留更多空间，并按表设置横向最小宽度。
- Grid 表头样式优化：表头加粗、居中，延续系统色彩与边框规范。
- 补回可点击行交互：访问成员表格的非操作单元格可继续打开成员编辑弹窗，号池表格的非操作单元格可继续打开号池配置弹窗。

## [2026-05-25 12:11 CST]

### 调整

- 账号资产主表试迁移到 Grid.js：桌面端 Codex 账号列表改为统一表格渲染，保留选择、账号来源 / 所有权、调用窗口、额度进度条、活动账号切换、置顶、同步额度和删除本地副本等现有操作。
- 账号资产主表保留原 `figma-table account-assets-table` 作为 Grid.js 加载失败时的 fallback，便于后续根据实际观感低成本撤回该区域迁移。

### 说明

- 本轮只调整账号资产展示层，不改账号资产存储、OAuth 凭据、号池成员引用、额度同步接口或任何本地持久化数据。
