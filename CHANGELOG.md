...
- 顶部状态栏入口改为“图标 + RG 文本”双标识，并补充 Tray 创建/图标加载日志，降低用户曾手动拖拽移除或菜单栏图标过小时无法确认入口的排障成本。
- 为顶部状态栏 Tray 设置新的稳定 `guid`，避免沿用旧状态栏项身份时继承用户曾手动拖拽移除/隐藏的 macOS 记忆状态。
- 新增顶部状态栏入口兜底：当 Electron Tray 已创建但 macOS 返回 `bounds.height=0`、状态项未进入可见菜单栏时，自动显示右上角 `RG` 悬浮入口，点击可打开控制台，并持续检测原生 Tray 恢复后自动隐藏兜底入口。
...
- 顶部状态栏兜底入口隐藏后不再重复写入隐藏日志，避免菜单栏可见状态下 `desktop-main.log` 被 1.5 秒轮询持续刷屏。
