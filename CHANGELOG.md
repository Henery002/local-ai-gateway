...
- 新增正式访问告警确认状态第一段：`access_alert_events` 兼容新增 `acknowledged_at / acknowledged_by`，管理端新增 `/admin/access/alerts/:id/acknowledge`，桌面端“用量与告警”可确认最近未确认告警；确认只作用于本项目本地告警事件，不影响账号、号池或 Cockpit / OpenClaw 原始配置。
...
- 新增 gateway 回归测试，覆盖成员日额度拒绝写入 `access_alert_events`、通过 `/admin/access/alerts` 查询、告警事件按行数裁剪，以及管理员确认告警后返回 `acknowledgedAt / acknowledgedBy`。
- 新增 gateway 与桌面构建回归测试，覆盖成员小时趋势聚合、用量页趋势图 hook 和桌面端告警确认入口。
